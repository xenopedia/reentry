...
###### PID control part#########
    ...
    if (time>60 and time<200):
        for i in range(0, 4):
            iv[i] = pv[i]

        timei = 0
        stepi = 0.1
        while math.sqrt(iv[0]*iv[0]+iv[2]*iv[2]) > ERAD:
            newstepi = rk.rk45(fall_force_model, timei, iv, stepi, 1E-7)
            timei += stepi
            stepi = newstepi
        final_i_angle =  math.pi/2 - math.atan2(iv[2], iv[0])
        if final_i_angle<0:
            final_i_angle = math.pi/2 - final_i_angle
        
        final_i_dist = ERAD * final_i_angle
        error = target - final_i_dist/1000.0
        integral = integral + error * step
        deriv = (error - pe)/step
        LoverD = 0.368 + 1e-6*error + 5e-5*integral + 1e-7*deriv
        pe = error
        ...
        if (LoverD<0):
            LoverD = 0
            integral = 0.0
        print (time, " (s) intermediary distance  ", final_i_dist/1000.0, " L/D ", LoverD )
...
######end control part#########
    ...
    #print (time, XE[-1], YE[-1], math.atan2(pv[2], pv[0])*180/math.pi)
